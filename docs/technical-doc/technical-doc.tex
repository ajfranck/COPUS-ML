...
\begin{itemize}
    ...
    \item Time-intensive process: 1-2 hours per lecture video
...
\textbf{Variable Video Quality:} Classroom videos vary in resolution, lighting, camera angle, and audio quality. (Not the case in this implementation, all videos are relatively similar)
...
\textbf{FFmpeg:}
\begin{itemize}
    \item Transcoding (MTS → MP4)
    \item Frame extraction at specific framerates
\end{itemize}
...
\textbf{Key Features:}
\begin{itemize}
    \item Automated full-lecture processing
...
\subsection{video\_utils.py}
\textbf{Purpose:} Video preprocessing utilities

\textbf{Key Functions:}
\begin{itemize}
    \item \texttt{convert\_mts\_to\_mp4()}: Format conversion using FFmpeg
    \item \texttt{extract\_frames\_at\_fps()}: Frame extraction at target framerate
    \item \texttt{get\_video\_metadata()}: Duration, resolution, codec info
    \item \texttt{validate\_video\_file()}: Check file existence and format
\end{itemize}

\textbf{When to modify:}
\begin{itemize}
    \item Supporting new video formats
    \item Optimizing frame extraction
    \item Adding video quality checks
    \item Implementing video downsampling
\end{itemize}
...
python video_utils.py --test-video tests/sample_videos/short_lecture.mp4

# Test model inference on single window
python full_lecture_evaluation.py --test-inference \
    --video tests/sample_videos/short_lecture.mp4 \
